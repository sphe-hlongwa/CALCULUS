...
**(e)** Let $\\displaystyle F(x)=\\int_0^{x^2}\\sin(t^3)\\,dt$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** Attempting to find an antiderivative of $\\sin(t^3)$ first is not productive — it has no elementary closed form. Instead, use the Fundamental Theorem of Calculus together with the Chain Rule: if $\\displaystyle F(x)=\\int_a^{u(x)}f(t)\\,dt$, then $F'(x)=f(u(x))\\cdot u'(x)$. ✓✓

Here $f(t)=\\sin(t^3)$ and $u(x)=x^2$, so $u'(x)=2x$. Substituting $t=u(x)=x^2$ into $f$:
$$F'(x)=\\sin\\!\\big((x^2)^3\\big)\\cdot2x=2x\\sin(x^6).$$ ✓
        `
      },
      {
        number: 6,
...
**(e)** Let $\\displaystyle F(x)=\\int_1^{x^3}\\sqrt{1+t^2}\\,dt$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** The integrand $\\sqrt{1+t^2}$ does have an elementary antiderivative, but it is a messy one — trying to find it first is the slow route. It is faster to apply the Fundamental Theorem of Calculus with the Chain Rule directly: if $\\displaystyle F(x)=\\int_a^{u(x)}f(t)\\,dt$, then $F'(x)=f(u(x))\\cdot u'(x)$. ✓✓

Here $f(t)=\\sqrt{1+t^2}$ and $u(x)=x^3$, so $u'(x)=3x^2$. Substituting $t=u(x)=x^3$:
$$F'(x)=\\sqrt{1+(x^3)^2}\\cdot3x^2=3x^2\\sqrt{1+x^6}.$$ ✓
        `
      },
      {
        number: 6,
...
**(e)** Let $\\displaystyle F(x)=\\int_2^{\\cos x}e^{t^2}\\,dt$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** The integrand $e^{t^2}$ has **no elementary antiderivative** at all, so direct integration is impossible here — this must be done with the Fundamental Theorem of Calculus and the Chain Rule: if $\\displaystyle F(x)=\\int_a^{u(x)}f(t)\\,dt$, then $F'(x)=f(u(x))\\cdot u'(x)$. ✓✓

Here $f(t)=e^{t^2}$ and $u(x)=\\cos x$, so $u'(x)=-\\sin x$. Substituting $t=u(x)=\\cos x$:
$$F'(x)=e^{\\cos^2x}\\cdot(-\\sin x)=-\\sin x\\,e^{\\cos^2x}.$$ ✓
        `
      },
      {
        number: 6,
...
**(e)** Let $\\displaystyle F(x)=\\int_x^{5}\\ln(1+t^2)\\,dt$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** Here the variable sits in the **lower** limit, and the upper limit is constant, so first flip the integral using $\\displaystyle\\int_x^{b}f(t)\\,dt=-\\int_b^xf(t)\\,dt$:
$$F(x)=-\\int_5^xf(t)\\,dt,\\qquad f(t)=\\ln(1+t^2).$$ ✓

Now $F(x)=-\\int_5^{u(x)}f(t)\\,dt$ with $u(x)=x$, so by the Fundamental Theorem of Calculus, $F'(x)=-f(u(x))\\cdot u'(x)=-f(x)\\cdot1$. ✓✓
$$F'(x)=-\\ln(1+x^2).$$ ✓
        `
      },
      {
...
**(e)** Let $\\displaystyle F(x)=\\int_0^{x^2}\\frac{dt}{1+t^4}$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** The integrand $\\dfrac{1}{1+t^4}$ has a real but very unwieldy elementary antiderivative — computing it is not the intended route. Instead use the Fundamental Theorem of Calculus with the Chain Rule: if $\\displaystyle F(x)=\\int_a^{u(x)}f(t)\\,dt$, then $F'(x)=f(u(x))\\cdot u'(x)$. ✓✓

Here $f(t)=\\dfrac{1}{1+t^4}$ and $u(x)=x^2$, so $u'(x)=2x$. Substituting $t=u(x)=x^2$:
$$F'(x)=\\frac{1}{1+(x^2)^4}\\cdot2x=\\frac{2x}{1+x^8}.$$ ✓
        `
      },
      {
        number: 6,
...
**(e)** Let $\\displaystyle F(x)=\\int_{\\sin x}^{2}\\sqrt{4-t^2}\\,dt$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** The variable is in the **lower** limit here, so first rewrite using $\\displaystyle\\int_{v(x)}^{b}f(t)\\,dt=-\\int_b^{v(x)}f(t)\\,dt$:
$$F(x)=-\\int_2^{\\sin x}\\sqrt{4-t^2}\\,dt.$$ ✓

Now apply the Fundamental Theorem of Calculus with the Chain Rule to the inner integral, with $f(t)=\\sqrt{4-t^2}$ and $v(x)=\\sin x$, so $v'(x)=\\cos x$: $F'(x)=-f(v(x))\\cdot v'(x)$. ✓✓
$$F'(x)=-\\sqrt{4-\\sin^2x}\\cdot\\cos x.$$ ✓
        `
      },
      {
        number: 6,
...
$$\\int\\cos^n x\\,dx=\\frac{\\cos^{n-1}x\\sin x}{n}+\\frac{n-1}{n}\\int\\cos^{n-2}x\\,dx.\\qquad\\blacksquare$$ ✓
     ...
     {
       number: 5, title: 'Inverse trigonometric, exponential and logarithmic integrals', section: '§9.1', marks: 9,
        prompt: `
...
**(e)** Let $\\displaystyle F(x)=\\int_x^{x^2}e^{-t^2}\\,dt$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** Both limits depend on $x$ here, so split the integral at any convenient constant, say $t=0$, and use the reversal property on the first piece:
$$F(x)=\\int_x^{x^2}e^{-t^2}\\,dt=\\int_x^{0}e^{-t^2}\\,dt+\\int_0^{x^2}e^{-t^2}\\,dt=-\\int_0^{x}e^{-t^2}\\,dt+\\int_0^{x^2}e^{-t^2}\\,dt.$$ ✓✓

Differentiating each piece by the Fundamental Theorem of Calculus with the Chain Rule, with $f(t)=e^{-t^2}$:
$$F'(x)=-f(x)\\cdot1+f(x^2)\\cdot2x=-e^{-x^2}+2x\\,e^{-x^4}.$$ ✓✓
        `
      },
      {
...
$$=\\frac35\\ln|x+2|+\\frac{7}{10}\\ln\\left(x^2+1\\right)+\\frac15\\arctan x+C.$$ ✓
  ...
  // =====================================================================
  {
    id: 'paper8', label: 'Practice Paper 8', date: 'Practice Test H', totalMarks: 55, duration: 65,
    questions: [
      {
...
**(e)** Let $\\displaystyle F(x)=\\int_2^{\\tan x}\\frac{dt}{1+t^3}$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** The integrand $\\dfrac{1}{1+t^3}$ can technically be split by partial fractions, but doing so here is unnecessary and slow — the Fundamental Theorem of Calculus with the Chain Rule handles this directly: if $\\displaystyle F(x)=\\int_a^{u(x)}f(t)\\,dt$, then $F'(x)=f(u(x))\\cdot u'(x)$. ✓✓

Here $f(t)=\\dfrac{1}{1+t^3}$ and $u(x)=\\tan x$, so $u'(x)=\\sec^2x$. Substituting $t=u(x)=\\tan x$:
$$F'(x)=\\frac{1}{1+\\tan^3x}\\cdot\\sec^2x=\\frac{\\sec^2x}{1+\\tan^3x}.$$ ✓
        `
      },
      {
...
$$\\int\\frac{x^3+2x^2+5}{x^2+1}\\,dx=\\frac{x^2}{2}+2x-\\frac12\\ln\\left(x^2+1\\right)+3\\arctan x+C.$$ ✓✓
  ...
  // =====================================================================
  {
    id: 'paper9', label: 'Practice Paper 9', date: 'Practice Test I', totalMarks: 55, duration: 65,
    questions: [
      {
...
**(e)** Let $\\displaystyle F(x)=\\int_0^{\\ln x}\\cos(t^2)\\,dt$ for $x>0$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** The integrand $\\cos(t^2)$ has no elementary antiderivative, so this must be done with the Fundamental Theorem of Calculus and the Chain Rule: if $\\displaystyle F(x)=\\int_a^{u(x)}f(t)\\,dt$, then $F'(x)=f(u(x))\\cdot u'(x)$. ✓✓

Here $f(t)=\\cos(t^2)$ and $u(x)=\\ln x$, so $u'(x)=\\dfrac1x$. Substituting $t=u(x)=\\ln x$:
$$F'(x)=\\cos\\!\\big((\\ln x)^2\\big)\\cdot\\frac1x=\\frac{\\cos\\!\\big((\\ln x)^2\\big)}{x}.$$ ✓
        `
      },
      {
...
$$\\int\\frac{x^3+x^2+2x+1}{\\left(x^2+1\\right)^2}\\,dx=\\frac12\\ln\\left(x^2+1\\right)+\\arctan x-\\frac{1}{2\\left(x^2+1\\right)}+C.$$ ✓
  ...
  // =====================================================================
  {
    id: 'paper10', label: 'Practice Paper 10', date: 'Practice Test J', totalMarks: 55, duration: 65,
    questions: [
      {
...
**(e)** Let $\\displaystyle F(x)=\\int_{x^2}^{x^3}\\sqrt{1+t^2}\\,dt$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** Both bounds depend on $x$, so split at a convenient constant, say $t=0$, and use the reversal property on the lower piece:
$$F(x)=\\int_{x^2}^{x^3}\\sqrt{1+t^2}\\,dt=\\int_{x^2}^{0}\\sqrt{1+t^2}\\,dt+\\int_0^{x^3}\\sqrt{1+t^2}\\,dt=-\\int_0^{x^2}\\sqrt{1+t^2}\\,dt+\\int_0^{x^3}\\sqrt{1+t^2}\\,dt.$$ ✓✓

Differentiating each piece by the Fundamental Theorem of Calculus with the Chain Rule, with $f(t)=\\sqrt{1+t^2}$:
$$F'(x)=-f(x^2)\\cdot2x+f(x^3)\\cdot3x^2=3x^2\\sqrt{1+x^6}-2x\\sqrt{1+x^4}.$$ ✓✓
        `
      },
      {
...
$$=2x-3\\ln|x|-\\frac4x+4\\ln|x+1|+C.$$ ✓
  ...
  // =====================================================================
  {
    id: 'paper11', label: 'Practice Paper 11', date: 'Practice Test K', totalMarks: 55, duration: 65,
    questions: [
      {
...
**(e)** Let $\\displaystyle F(x)=\\int_1^{e^x}\\sqrt{t^2+1}\\,dt$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** The integrand $\\sqrt{t^2+1}$ has a real antiderivative, but it is far quicker to differentiate directly with the Fundamental Theorem of Calculus and the Chain Rule: if $\\displaystyle F(x)=\\int_a^{u(x)}f(t)\\,dt$, then $F'(x)=f(u(x))\\cdot u'(x)$. ✓✓

Here $f(t)=\\sqrt{t^2+1}$ and $u(x)=e^x$, so $u'(x)=e^x$. Substituting $t=u(x)=e^x$:
$$F'(x)=\\sqrt{(e^x)^2+1}\\cdot e^x=e^x\\sqrt{e^{2x}+1}.$$ ✓
        `
      },
      {
...
$$=-\\ln|x-1|+\\ln|x+2|+2\\ln|x-3|+C.$$ ✓
  ...
  // =====================================================================
  {
    id: 'paper12', label: 'Practice Paper 12', date: 'Practice Test L', totalMarks: 55, duration: 65,
    questions: [
      {
...
**(e)** Let $\\displaystyle F(x)=\\int_{\\sqrt x}^{4}t\\cos t\\,dt$ for $x>0$. Find $F'(x)$. (3)
        `,
        solution: `
...
**(e)** The variable is in the **lower** limit here, so first flip using $\\displaystyle\\int_{v(x)}^{b}f(t)\\,dt=-\\int_b^{v(x)}f(t)\\,dt$:
$$F(x)=-\\int_4^{\\sqrt x}t\\cos t\\,dt.$$ ✓

By the Fundamental Theorem of Calculus with the Chain Rule, with $f(t)=t\\cos t$ and $v(x)=\\sqrt x$, so $v'(x)=\\dfrac{1}{2\\sqrt x}$: $F'(x)=-f(v(x))\\cdot v'(x)$. ✓✓
$$F'(x)=-\\sqrt x\\cos\\!\\left(\\sqrt x\\right)\\cdot\\frac{1}{2\\sqrt x}=-\\frac12\\cos\\!\\left(\\sqrt x\\right),$$
where the $\\sqrt x$ factors cancel neatly. ✓
        `
      },
      {
